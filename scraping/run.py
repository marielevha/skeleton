...
class RunScraper:
    ...
    def save_in_db(self, el):
        return Announce.objects.create(
    ...
    def save_data_to_db(self):
        ...
        for el in self.ma_final_data:
            ad = self.save_in_db(el)
            print(ad)
        # SAVE AVITO DATA
        # for el in self.avito_final_data:
        #     ad = self.save_in_db(el)
        #     print(ad)
...
def launch_scraping():
    ...
    # # db_thread = threading.Thread(target=runner.save_data_to_db)
    #
    # ma_thread.start()
    # # avito_thread.start()
    # ma_thread.join()
